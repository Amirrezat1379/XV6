  ...
  aa:	31 db                	xor    %ebx,%ebx
{
  ...
  fc:	74 42                	je     140 <wc+0xa0>
      if(buf[i] == '\n')
  fe:	0f be 87 40 0d 00 00 	movsbl 0xd40(%edi),%eax
        l++;
 ...
 4ec:	89 d1                	mov    %edx,%ecx
{
 ...
 519:	0f b6 92 d4 09 00 00 	movzbl 0x9d4(%edx),%edx
...
printf(int fd, const char *fmt, ...)
{
 ...
 714:	eb 1a                	jmp    730 <printf+0x1a0>
          s = "(null)";
 716:	bb cb 09 00 00       	mov    $0x9cb,%ebx
        while(*s != 0){
...
free(void *ap)
{
 ...
 765:	a1 20 0d 00 00       	mov    0xd20,%eax
{
 ...
 786:	73 10                	jae    798 <free+0x38>
{
 ...
 7b1:	5b                   	pop    %ebx
  freep = p;
 7b2:	a3 20 0d 00 00       	mov    %eax,0xd20
}
 7b7:	5e                   	pop    %esi
 7b8:	5f                   	pop    %edi
 7b9:	5d                   	pop    %ebp
 ...
 7d7:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 7da:	a3 20 0d 00 00       	mov    %eax,0xd20
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 7fd:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 800:	8b 3d 20 0d 00 00    	mov    0xd20,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 849:	8b 3d 20 0d 00 00    	mov    0xd20,%edi
 ...
 872:	e8 e9 fe ff ff       	call   760 <free>
  return freep;
 877:	a1 20 0d 00 00       	mov    0xd20,%eax
 ...
 8a4:	89 72 04             	mov    %esi,0x4(%edx)
      freep = prevp;
 8a7:	a3 20 0d 00 00       	mov    %eax,0xd20
}
 ...
 8be:	66 90                	xchg   %ax,%ax
    base.s.ptr = freep = prevp = &base;
 8c0:	c7 05 20 0d 00 00 24 	movl   $0xd24,0xd20
 8c7:	0d 00 00 
    base.s.size = 0;
 8ca:	bf 24 0d 00 00       	mov    $0xd24,%edi
    base.s.ptr = freep = prevp = &base;
 8cf:	c7 05 24 0d 00 00 24 	movl   $0xd24,0xd24
 8d6:	0d 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 8d9:	89 f8                	mov    %edi,%eax
    base.s.size = 0;
 8db:	c7 05 28 0d 00 00 00 	movl   $0x0,0xd28
 8e2:	00 00 00 
    if(p->s.size >= nunits){
...
int thread_creator (void (*fn) (void *), void *args) {
 ...
 907:	53                   	push   %ebx
 908:	83 ec 20             	sub    $0x20,%esp
    void *fptr = malloc(2 * PAGESIZE);
 90b:	68 00 20 00 00       	push   $0x2000
 910:	e8 db fe ff ff       	call   7f0 <malloc>
    void *stack;

    if (fptr == 0)
 915:	83 c4 10             	add    $0x10,%esp
 918:	85 c0                	test   %eax,%eax
 91a:	74 59                	je     975 <thread_creator+0x75>
 91c:	89 c3                	mov    %eax,%ebx
        return -1;
    
    int mod = (uint)fptr % PAGESIZE;

    if (mod == 0)
 91e:	25 ff 0f 00 00       	and    $0xfff,%eax
 923:	75 1b                	jne    940 <thread_creator+0x40>
        stack = fptr;
    else
 ...
 928:	53                   	push   %ebx
 929:	e8 7d fb ff ff       	call   4ab <thread_create>

    if (thread_id < 0)
 92e:	83 c4 10             	add    $0x10,%esp
 931:	85 c0                	test   %eax,%eax
 933:	78 23                	js     958 <thread_creator+0x58>
        printf(1, "Thread create faild :( !\n");
    else if (thread_id == 0) {
 935:	74 45                	je     97c <thread_creator+0x7c>
        (fn)(args);
        free(stack);
        exit();
    }
 ...
 93c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        stack = fptr + (PAGESIZE - mod);
 940:	29 c3                	sub    %eax,%ebx
    int thread_id = thread_create((void*)stack);
 942:	83 ec 0c             	sub    $0xc,%esp
        stack = fptr + (PAGESIZE - mod);
 945:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    int thread_id = thread_create((void*)stack);
 94b:	53                   	push   %ebx
 94c:	e8 5a fb ff ff       	call   4ab <thread_create>
    if (thread_id < 0)
 951:	83 c4 10             	add    $0x10,%esp
 954:	85 c0                	test   %eax,%eax
 956:	79 dd                	jns    935 <thread_creator+0x35>
        printf(1, "Thread create faild :( !\n");
 958:	83 ec 08             	sub    $0x8,%esp
 95b:	89 45 f4             	mov    %eax,-0xc(%ebp)
 95e:	68 e5 09 00 00       	push   $0x9e5
 963:	6a 01                	push   $0x1
 965:	e8 26 fc ff ff       	call   590 <printf>
 ...
 97a:	eb bb                	jmp    937 <thread_creator+0x37>
        (fn)(args);
 97c:	83 ec 0c             	sub    $0xc,%esp
 97f:	ff 75 0c             	pushl  0xc(%ebp)
 982:	ff 55 08             	call   *0x8(%ebp)
        free(stack);
 985:	89 1c 24             	mov    %ebx,(%esp)
 988:	e8 d3 fd ff ff       	call   760 <free>
        exit();
 98d:	e8 61 fa ff ff       	call   3f3 <exit>
